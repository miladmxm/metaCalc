...
import { getIndexesHttp } from "../../services/HTTP";
const Home = () => {
  const { t } = useTranslation();
  const [activeIndexId, setActiveIndexId] = useState(0);
  const { data, isSuccess } = useQuery({
    queryKey: [constant.INDEXES],
    queryFn: getIndexesHttp,
  });
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-5">
        {Indexes?.map((index) => (
          <button
            onClick={() => setActiveIndexId(index._id)}
            className={cn(
              "hover:ring-2 transition-all duration-300 ring-success p-2 flex-1 max-w-20 rounded-xl text-white",
              {
                "bg-primary": activeIndexId === index._id,
                "bg-gray-800/50": activeIndexId !== index._id,
              }
            )}
            key={index.name}
          >
