  ...
  return (
    <div className="uppercase">
      <div className="ring-2 ring-text h-7 rounded-lg center w-auto">
        <button
          onClick={() => setDisplayLangs((pre) => !pre)}
          className="w-7 center"
        >
          <svg
            stroke="currentColor"
            fill="currentColor"
        ...
        <div
          className={cn(
            "overflow-hidden flex justify-around transition-all duration-300",
            { "w-16": displayLangs, "w-0": !displayLangs }
          )}
        >
          {supportedLngs.map((lang) => (
            <button
              className={cn("uppercase center font-Inter", {
